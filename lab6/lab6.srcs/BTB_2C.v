...
module BTB_2C (
    ...
    reg [`BTB_ENTRY_2C - 1:0] btb [0:`BTB_SIZE - 1];

    wire [`BTB_ENTRY_2C - 1:0] entry;
    wire [`BTB_ENTRY_2C - 1:0] update_entry;
    ...
    assign predpc = (pc[`WORD_SIZE - 1-:`BTB_TAG] == entry[`BTB_ENTRY_2C - 1-:`BTB_TAG] && entry[`WORD_SIZE] && entry[`WORD_SIZE + 2]) ? entry[`WORD_SIZE - 1:0] : pcplusone;

    always @(posedge clk or negedge reset_n) begin
        if (!reset_n) begin
            for (i = 0; i < `BTB_SIZE; i = i + 1) begin
                btb[i] <= `BTB_ENTRY_2C'b0;
            end
        end
        else begin
            if (update_btb) begin
                if (update_pc[`WORD_SIZE - 1-:`BTB_TAG] == update_entry[`BTB_ENTRY_2C - 1-:`BTB_TAG] && update_entry[`WORD_SIZE]) begin
                    if (update_nextpc == update_entry[`WORD_SIZE - 1:0]) begin
                        btb[update_pc[`BTB_ADDR - 1:0]] <= {update_entry[`BTB_ENTRY_2C - 1-:`BTB_TAG], update_counter_plus, 1'b1, update_entry[`WORD_SIZE - 1:0]};
                    end
                    else begin
                        btb[update_pc[`BTB_ADDR - 1:0]] <= {update_entry[`BTB_ENTRY_2C - 1-:`BTB_TAG], update_counter_minus, 1'b1, update_entry[`WORD_SIZE - 1:0]};
                    end
                end
                else begin
